...
def send_message(name, message_body, message_attributes=None):
    ...
    try:
        ...
        return response
# print(send_message('SecondQueueekor','hi! himanshu kesharwani good afternoon'))
...
def send_messages(name):
    ...
    try:
        entries = [{
            'Id': str(1),
            'MessageBody': 'Himanshu works in protonshub'
        },{
            'Id': str(2),
            'MessageBody': 'Himanshu have 2 years of experience'
        }]
        queue = sqs.get_queue_by_name(QueueName=name)
        ...
        return response
# print(send_messages('SecondQueueekor'))

def receive_messages(name, max_number, wait_time):
    """
    ...
    try:
        ...
        for msg in messages:
            logger.critical("Received message: %s: %s", msg.message_id, msg.body)
    except ClientError as error:
        logger.exception("Couldn't receive messages from queue: %s", queue)
        raise error
    else:
        return messages
# print(receive_messages('SecondQueueekor',5,0))
...
delete_message('SecondQueueekor')
